...
std::shared_ptr<::mls::SignaturePrivateKey> GetNativePersistedKeyPair(KeyPairContextType ctx,
                                                                        ...
                                                                        bool& supported)
{
    std::shared_ptr<::mls::SignaturePrivateKey> ret;
    ...
    switch (suiteId) {
    case ::mls::CipherSuite::ID::P256_AES128GCM_SHA256_P256:
    case ::mls::CipherSuite::ID::P384_AES256GCM_SHA384_P384:
    case ::mls::CipherSuite::ID::P521_AES256GCM_SHA512_P521:
        supported = true;
        keyType = kSecAttrKeyTypeECSECPrimeRandom;
        if (suiteId == ::mls::CipherSuite::ID::P521_AES256GCM_SHA512_P521) {
            keySize = 521;
        }
        else if (suiteId == ::mls::CipherSuite::ID::P384_AES256GCM_SHA384_P384) {
            keySize = 384;
        }
        else {
            keySize = 256;
    ...
    return std::make_shared<::mls::SignaturePrivateKey>(
      ::mls::SignaturePrivateKey::parse(suite, converted));
}

std::shared_ptr<::mls::SignaturePrivateKey> GetGenericPersistedKeyPair(KeyPairContextType ctx,
                                                                         const std::string& id,
                                                                         ::mls::CipherSuite suite)
{
    ::mls::SignaturePrivateKey ret;
    ...
    for (int attempt = 0; attempt < AttemptCount && ret.public_key.data.empty(); attempt++) {
        ...
        if (status == 0 && result) {
            ...
            try {
                ret = ::mls::SignaturePrivateKey::from_jwk(suite, curstr);
            }
            ...
            ret = ::mls::SignaturePrivateKey::generate(suite);
    ...
    if (!ret.public_key.data.empty()) {
        return std::make_shared<::mls::SignaturePrivateKey>(std::move(ret));
    }
    else {
        return nullptr;
    }
