...
try {
    ...
    DISCORD_LOG(LS_INFO) << "Sender: " << ::mls::bytes_ns::bytes(marshalledExternalSender);

    externalSender_ = std::make_unique<::mls::ExternalSender>(
      ::mls::tls::get<::mls::ExternalSender>(marshalledExternalSender));
...
try {
    ...
    if (!stateWithProposals_) {
        stateWithProposals_ = std::make_unique<::mls::State>(
    ...
    DISCORD_LOG(LS_INFO) << "Proposals: " << ::mls::bytes_ns::bytes(proposals);

    ::mls::tls::istream inStream(proposals);
    ...
    if (isRevoke) {
        std::vector<::mls::bytes_ns::bytes> refs;
        inStream >> refs;
    ...
    // generate a commit
    auto commitSecret = ::mls::hpke::random_bytes(suite.secret_size());

    auto commitOpts = ::mls::CommitOpts{
      {},    // no extra proposals
    ...
    outboundCachedGroupState_ = std::make_unique<::mls::State>(std::move(newState));

    DISCORD_LOG(LS_INFO) << "Output: " << ::mls::bytes_ns::bytes(outStream.bytes());
...
bool Session::IsRecognizedUserID(const ::mls::Credential& cred,
...
bool Session::ValidateProposalMessage(::mls::AuthenticatedContent const& message,
                                      ::mls::State const& targetState,
                                      std::set<std::string> const& recognizedUserIDs) const
{
    if (message.wire_format != ::mls::WireFormat::mls_public_message) {
    ...
    if (message.content.content_type() != ::mls::ContentType::proposal) {
        DISCORD_LOG(LS_ERROR) << "ProcessProposals called with non-proposal message";
        TRACK_MLS_ERROR("Unexpected message type");
        return false;
    }

    if (message.content.sender.sender_type() != ::mls::SenderType::external) {
        DISCORD_LOG(LS_ERROR) << "MLS proposal must be from external sender";
        TRACK_MLS_ERROR("Unexpected proposal sender type");
        return false;
    }

    const auto& proposal = ::mls::tls::var::get<::mls::Proposal>(message.content.content);
    switch (proposal.proposal_type()) {
    case ::mls::ProposalType::add: {
        const auto& credential =
          ::mls::tls::var::get<::mls::Add>(proposal.content).key_package.leaf_node.credential;
...
bool Session::CanProcessCommit(const ::mls::MLSMessage& commit) noexcept
{
    if (!stateWithProposals_) {
        return false;
    }
...
try {
    ...
    DISCORD_LOG(LS_INFO) << "Commit: " << ::mls::bytes_ns::bytes(commit);

    auto commitMessage = ::mls::tls::get<::mls::MLSMessage>(commit);
    ...
    std::optional<::mls::State> optionalCachedState = std::nullopt;
    ...
    RosterMap ret = ReplaceState(std::make_unique<::mls::State>(std::move(*newState)));
...
try {
    ...
    DISCORD_LOG(LS_INFO) << "Processing welcome: " << ::mls::bytes_ns::bytes(welcome);

    // unmarshal the incoming welcome
    auto unmarshalledWelcome = ::mls::tls::get<::mls::Welcome>(welcome);

    // construct the state from the unmarshalled welcome
    auto newState = std::make_unique<::mls::State>(
      *joinInitPrivateKey_,
      *selfHPKEPrivateKey_,
      *selfSigPrivateKey_,
      *joinKeyPackage_,
      unmarshalledWelcome,
      std::nullopt,
      std::map<::mls::bytes_ns::bytes, ::mls::bytes_ns::bytes>());
...
RosterMap Session::ReplaceState(std::unique_ptr<::mls::State>&& state)
{
    RosterMap newRoster;
    for (const ::mls::LeafNode& node : state->roster()) {
        if (node.credential.type() != ::mls::CredentialType::basic) {
            continue;
        }

        const auto& cred = node.credential.template get<::mls::BasicCredential>();
...
bool Session::VerifyWelcomeState(::mls::State const& state,
                                 std::set<std::string> const& recognizedUserIDs) const
{
    if (!externalSender_) {
    ...
    auto ext = state.extensions().template find<::mls::ExternalSendersExtension>();
    if (!ext) {
...
try {
    ...
    if (!transientKey) {
        if (!signingKeyId_.empty()) {
            ...
            transientKey = std::make_shared<::mls::SignaturePrivateKey>(
              ::mls::SignaturePrivateKey::generate(ciphersuite));
        }
    }
    ...
    selfHPKEPrivateKey_ =
      std::make_unique<::mls::HPKEPrivateKey>(::mls::HPKEPrivateKey::generate(ciphersuite));

    selfLeafNode_ =
      std::make_unique<::mls::LeafNode>(ciphersuite,
...
try {
    ...
    joinKeyPackage_ =
      std::make_unique<::mls::KeyPackage>(ciphersuite,
    ...
    DISCORD_LOG(LS_INFO) << "Generated key package: "
                         << ::mls::bytes_ns::bytes(::mls::tls::marshal(*joinKeyPackage_));
}
catch (const std::exception& e) {
...
try {
    ...
    pendingGroupState_ = std::make_unique<::mls::State>(
      groupId_,
      ciphersuite,
      *selfHPKEPrivateKey_,
      *selfSigPrivateKey_,
...
try {
    ...
    return ::mls::tls::marshal(*joinKeyPackage_);
}
catch (const std::exception& e) {
...
std::unique_ptr<IKeyRatchet> Session::GetKeyRatchet(std::string const& userId) const noexcept
{
    ...
    auto userIdBytes = ::mls::bytes_ns::bytes(sizeof(u64userId));
...
try {
    ...
    ::mls::tls::ostream toHash1;
    ::mls::tls::ostream toHash2;
    ...
    std::thread([callback = std::move(callback),
                 data = ::mls::bytes_ns::bytes(std::move(keyData[0])) + keyData[1]] {
